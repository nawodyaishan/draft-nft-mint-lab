 ...
 */
contract Minter is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function mint(address account, uint256 amount) external payable {
        ...
        require(totalSupply() + amount <= MAX_NFTS, "Minter: Purchase would exceed cap");
        require(maxPerWallet <= 0 || balanceOf(account) + amount <= maxPerWallet, "Minter: Maximum mints per wallet is exceeded");
        require((saleIsActive ? mintPrice : presaleMintPrice) * amount <= msg.value, "Minter: Ether value sent is not correct");
    ...
    function mintForCommunity(address _to, uint256 _numberOfTokens) external onlyOwner {
        ...
        require(totalSupply() + _numberOfTokens <= MAX_NFTS, "Minter: Minting would exceed cap");
